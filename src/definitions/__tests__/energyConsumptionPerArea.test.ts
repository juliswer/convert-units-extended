...
test('kWh/ft2 to kWh/in2', () => {
    const convert = configureMeasurements<'energyConsumptionPerArea', EnergyConsumptionPerAreaSystems, EnergyConsumptionPerAreaUnits>({
...
test('kBtu/ft2 to kWh/m2', () => {
    ...
    expect(convert(valueToTest).from('kBtu/ft2').to('kWh/m2')).toBeCloseTo(valueToTest * 0.293071 * 10.7639)
});
...
test('kWh/m2 to kBtu/ft2', () => {
    ...
    // Conversión de kWh/m² a kBtu/ft²
    expect(convert(valueToTest).from('kWh/m2').to('kBtu/ft2')).toBeCloseTo(valueToTest * (1/10.7639) * (1 / 0.293071)); // Aproximadamente 3.175 kBtu/ft²
});
